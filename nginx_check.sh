...
echo ""
echo " =========================================================================== "
echo " \                 Nginx Security Events Analysis Scenario V1.0            / "
echo " =========================================================================== "
echo " # Supports searching for Nginx log analisys, attack detection etc           "
echo " # author：al0ne                                                             "
echo " # https://github.com/al0ne                                                  "
echo " # Translated: Alexander Tyutin                                              "
echo " # https://github.com/AlexanderTyutin/                                       "
echo -e "\n"

#This script was changed according to nmgxy/klionsec and some funtions was added again as temporary measures.
#It is strongly recommended to check it with ELK or Splunk

#Functions
###Top 20 addresses statistics
###SQL Injections Analysis
###SQL Injections in FROM Requests
###Scanners / Common Hack Tools
###Exploits
###Sensitive Access Paths
###Attack File
###HTTP Tunnel
###Webshell
###Top 20 URL Response Lenght
###Rare Access to Script File
###Search 302 Error File

#如果存在多个access文件或者有多个access.x.gz 建议先zcat access*.gz >> access.log文件中
#It is recommended to concatenate compressed logs (access.X.gz) to one text file with zcat access*.gz >> access.log
#设置分析结果存储目录,结尾不能加/
#Setting results directory
outfile=/tmp/logs
#如果目录以存在则清空，未存在则新建目录
#Clear directory if it is not empty / create new if it doesn't exist
if [ -d $outfile ]; then
    rm -rf $outfile/*
else
    mkdir -p $outfile
...
#设置nginx日志目录，结尾必须加/
#Add / to Nginx log directory path
access_dir=/var/log/nginx/
#设置文件名，如果文件名为access那么匹配的是access*文件
#Insert name of log file. Default is access
access_log=access
#判断日志文件是否存在
...
# Check if the OS is Debian or CentOS
OS='None'
...
if ag -V >/dev/null 2>&1; then
    ...
    echo -e "\e[00;32msilversearcher-ag installed \e[00m"
else
    if [ $OS = 'Centos' ]; then
...
# If your log format differs from default, please correct this script manually.
# For example, in this file:
# - awk's $7 stands for URL
# - awk's $9 stands for status code
# - awk's $10 stands for length

#echo "分析结果日志：${outfile}"
echo "Analysis results file：${outfile}"
#echo "Nginx日志目录：${access_dir}"
echo "Nginx log directory：${access_dir}"
#echo "Nginx文件名：${access_log}"
echo "Nginx log file name：${access_log}"
echo -e "\n"
...
# Focus on the from query, whether there is any pants off, and exclude scanning
#echo "SQL注入 FROM 查询"
...
awk '{print "A total f scan attacks detected" NR"times"}' ${outfile}/scan.log | tail -n1
...
echo "Sensitive files access traffic TOP 20"
...
awk '{print "Exploit detected for" NR"times"}' ${outfile}/exploit.log | tail -n1
#echo "漏洞利用检测 TOP 20"
...
awk '{print "webshell was detected for " NR "times"}' ${outfile}/webshell.log | tail -n1
...
awk '{print "HTTP Tunnel was detected" NR "times"}' ${outfile}/tunnel.log | tail -n1
echo -e "\n"
...
echo -e "\e[00;31m[+]Rare Script Access\e[00m"
